...
measure_id = get_script_run_ctx().session_id
...
with main_cols[0]:
    ...
    with st.expander("Inputs", expanded=True):
        ...
        with filter_rows[2]:
            estimated_useful_life = st.number_input("EUL", value=10)
            discount_rate_ratio = st.number_input("Discount Rate", value=0.075)

        with filter_rows[3]:
            unit_quantity = st.number_input("Units", value=1, step=1)
            net_to_gross_ratio = st.number_input("NTG", value=1.0, step=0.01)

        costs_col = st.columns(3)
        with costs_col[0]:
            admin_cost_dollars = st.number_input("Administrative Cost", value=3000)
        with costs_col[1]:
            incentive_cost_dollars = st.number_input("Incentive Cost", value=500)
        with costs_col[2]:
            measure_cost_dollars = st.number_input("Measure Cost", value=1000)
        ...
        with commodity_input_tabs[0]:
            ...
            with elec_commodity_cols[1]:
                elec_savings_mwh = st.number_input("Annual MWh saving", value=10)
        ...
        with commodity_input_tabs[1]:
            ...
            with gas_commodity_cols[1]:
                gas_saving_therms = st.number_input("Annual Therms saving", value=100)
        ...
        update_measure(
            ...
            discount_rate_ratio=discount_rate_ratio, estimated_useful_life=estimated_useful_life,
            unit_quantity=unit_quantity, net_to_gross_ratio=net_to_gross_ratio,
...
with main_cols[1]:
    ...
    result = get_measure_impacts(measure_id)

    with st.container():
        row_top = st.columns(2)
        with row_top[0]:
            render_metric("Net Lifecycle MWh Savings", f"{result[MEASURE_IMPACT_NET_ELECTRIC_ENERGY_SAVINGS]:,.0f}")
        with row_top[1]:
            render_metric("Net Lifecycle Therm Savings", f"{result[MEASURE_IMPACT_NET_GAS_ENERGY_SAVINGS]:,.0f}")
        ...
        with row_mid[0]:
            render_metric(
                "Total System Benefit", f"{result[MEASURE_IMPACT_TOTAL_BENEFITS]:,.0f}",
                f"{result[MEASURE_IMPACT_ELECTRIC_BENEFITS]:,.0f}",
                f"{result[MEASURE_IMPACT_GAS_BENEFITS]:,.0f}"
            )
        with row_mid[1]:
            st.markdown(f"""<div style='padding: 10px; border: 1px solid #ccc; border-radius: 10px;'>
                <strong>TSB/MWh</strong><br><span style='font-size: 24px;'>{f"{result[MEASURE_IMPACT_TOTAL_BENEFITS_PER_MWH]:,.2f}"}</span><br>
                <small>TSB/Therm {f"{result[MEASURE_IMPACT_TOTAL_BENEFITS_PER_THERM]:,.2f}"}</small>
            </div>""", unsafe_allow_html=True)
        with row_mid[2]:
            render_metric(
                "GHG Savings (Tons)", f"{result[MEASURE_IMPACT_TOTAL_GHG_BENEFITS]:,.0f}",
                f"{result[MEASURE_IMPACT_ELECTRIC_GHG_BENEFITS]:,.0f}",
                f"{result[MEASURE_IMPACT_GAS_GHG_BENEFITS]:,.0f}"
            )
        with row_mid[3]:
            render_metric("TRC", f"{result[MEASURE_IMPACT_TRC_RATIO]:,.2f}")
        with row_mid[4]:
            render_metric("PAC", f"{result[MEASURE_IMPACT_PAC_RATIO]:,.2f}")

    energy_tabs = st.tabs(["Electricity", "Gas"])

    with energy_tabs[0]:
        electric_chart_data = get_electricity_impacts_by_avoided_cost_ts(measure_id, electricity_impact_selection)
    ...
    with energy_tabs[1]:
        gas_chart_data = get_gas_impacts_by_avoided_cost_ts(measure_id, gas_impact_selection)
