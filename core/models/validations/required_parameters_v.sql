...
SELECT
    ...
    , SUM(CASE WHEN net_to_gross_ratio IS NULL THEN 1 ELSE 0 END) AS count_null_net_to_gross_ratio
    ...
    , SUM(CASE WHEN COALESCE(m.measure_specific_discount_rate, gp.discount_rate) IS NULL THEN 1 ELSE 0 END) AS count_null_discount_rate
FROM
