...
run:
	...
	@duckdb output/duckdb.db -c "COPY (SELECT * FROM flexvalue.project_value_stream_benefits) TO 'output/project_benefits.csv' WITH (FORMAT CSV, HEADER TRUE);"
...
refresh-ref-output:
	duckdb output/duckdb.db "COPY flexvalue.project_value_stream_benefits TO 'test_data/test_real_data_calculations_aggregated/rdc_output_table.csv' (HEADER, DELIMITER ',');"
