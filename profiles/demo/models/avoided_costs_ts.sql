...
CROSS JOIN (
    SELECT unnest(ARRAY['total', 'marginal_ghg', 'energy', 'losses', 'ancillary_services', 'capacity']) AS cost_type
) AS vs
...
CROSS JOIN (
    SELECT unnest(ARRAY['total', 'marginal_ghg', 'market', 't_d', 'environment', 'btm_methane', 'upstream_methane']) AS cost_type
) AS vs;
