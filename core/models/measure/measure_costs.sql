MODEL(
    name measure.measure_cost_dollarss,
    kind VIEW,
    grain (measure_id),
);
SELECT
    ...
    admin_cost_dollars_dollars
        + (((1 - net_to_gross_ratio) * incentive_cost_dollars) + (net_to_gross_ratio * measure_cost_dollars))
        / (1 + (discount_rate_ratio / 4.0))
        AS trc_cost_dollars,
    admin_cost_dollars_dollars + (incentive_cost_dollars / (1 + (discount_rate_ratio / 4.0))) as pac_cost_dollars,
FROM
