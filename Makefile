...
run:
	...
	@duckdb output/${PROFILE}.db -c "COPY (SELECT * FROM openbca.project_value_stream_benefits) TO 'output/project_benefits_${PROFILE}.csv' WITH (FORMAT CSV, HEADER TRUE);"
...
check-output:
	...
	sqlmesh table_diff openbca.project_value_stream_benefits:openbca.rdc_output_table -o project_id --show-sample

refresh-ref-output:
	duckdb output/duckdb.db "COPY openbca.project_value_stream_benefits TO 'california/data/test_real_data_calculations_aggregated/rdc_output_table.csv' (HEADER, DELIMITER ',');"
