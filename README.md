...
Project input data are sourced from the `input/measures.csv`. Other reference files are sourced from `data/test_real_data_calculations_aggregated`.
...
A CSV with the final results is automatically extracted from DuckDB using the command `make run`: `output/measure_benefits.csv`.
...
To run the OpenBCA calculation, we need the following inputs:
 - `input/measures.csv`: the measure input data
